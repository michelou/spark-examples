...
if %_LANG%==java ( set _APP_NAME=JavaWordCounts
) else ( set _APP_NAME=WordCounts
)
...
set "__INPUT1_FILE=%_TARGET_DIR%\input1.txt"
set "__INPUT2_FILE=%_TARGET_DIR%\input2.txt"
(
    echo A car is driving down the street.
) > "%__INPUT1_FILE%"
(
    echo A car is driving down the street.
) > "%__INPUT2_FILE%"

@rem options: --driver-java-options "-Denv=DEV"
